...
QUnit.test("ALU BUS", function(assert) {
  assert.ok(12 === Alu.execute(AluFunction.BUS, 12, 0, 0), "bus == 12");
...
QUnit.test("ALU BUS_OR_T", function(assert) {
  assert.ok(0xF === Alu.execute(AluFunction.BUS_OR_T, 5, 0xa, 0), "5|a == f");
  assert.ok(0xF === Alu.execute(AluFunction.BUS_OR_T, 3, 0xc, 0), "3|c == f");
  assert.ok(7 === Alu.execute(AluFunction.BUS_OR_T, 2, 5, 0), "2|5 == 7");
  assert.ok(0 === Alu.carry, "carry == 0");
});

QUnit.test("ALU BUS_AND_T", function(assert) {
  assert.ok(0 === Alu.execute(AluFunction.BUS_AND_T, 5, 0xa, 0), "5&a == 0");
  assert.ok(0 === Alu.execute(AluFunction.BUS_AND_T, 0xa, 5, 0), "a&5 == 0");
  assert.ok(4 === Alu.execute(AluFunction.BUS_AND_T, 0xc, 7, 0), "3&c == 4");
  assert.ok(6 === Alu.execute(AluFunction.BUS_AND_T, 6, 0xf, 0), "6&f == 6");
  assert.ok(0 === Alu.carry, "carry == 0");
});

QUnit.test("ALU ALU_BUS_AND_T", function(assert) {
  assert.ok(0 === Alu.execute(AluFunction.ALU_BUS_AND_T, 5, 0xa, 0), "5&a == 0");
  assert.ok(0 === Alu.execute(AluFunction.ALU_BUS_AND_T, 0xa, 5, 0), "a&5 == 0");
  assert.ok(4 === Alu.execute(AluFunction.ALU_BUS_AND_T, 0xc, 7, 0), "3&c == 4");
  assert.ok(6 === Alu.execute(AluFunction.ALU_BUS_AND_T, 6, 0xf, 0), "6&f == 6");
  assert.ok(0 === Alu.carry, "carry == 0");
});

QUnit.test("ALU BUS_XOR_T", function(assert) {
  assert.ok(5 === Alu.execute(AluFunction.BUS_XOR_T, 0xa, 0xf, 0), "a^f == 5");
  assert.ok(0 === Alu.execute(AluFunction.BUS_XOR_T, 0xf, 0xf, 0), "f^f == 0");
  assert.ok(4 === Alu.execute(AluFunction.BUS_XOR_T, 3, 7, 0), "3^7 == 4");
  assert.ok(4 === Alu.execute(AluFunction.BUS_XOR_T, 7, 3, 0), "7^3 == 4");
  assert.ok(0 === Alu.carry, "carry == 0");
});
...
QUnit.test("ALU BUS_MINUS_T", function(assert) {
  assert.ok(0xe === Alu.execute(AluFunction.BUS_MINUS_T, 0xf, 1, 0), "f - 1 == e");
  // Subtraction carry is reversed from intuition!
  assert.ok(1 === Alu.carry, "carry == 1");
  ...
  // Carry should work.
  assert.ok(0 === Alu.execute(AluFunction.BUS_MINUS_T, 1, 1, 0), "1 - 1 == 0");
  assert.ok(1 === Alu.carry, "carry == 1");
  assert.ok(0xffff === Alu.execute(AluFunction.BUS_MINUS_T, 0, 1, 0), "0 - 1 == ffff");
  assert.ok(0 === Alu.carry, "carry == 0");
});

QUnit.test("ALU BUS_MINUS_T_MINUS_1", function(assert) {
  assert.ok(0xd === Alu.execute(AluFunction.BUS_MINUS_T_MINUS_1, 0xf, 1, 0),
            "f - 1 - 1 == d");
  // Subtraction carry is reversed from intuition!
  assert.ok(1 === Alu.carry, "carry == 1");

  assert.ok(0xffef === Alu.execute(AluFunction.BUS_MINUS_T_MINUS_1, 0xffff, 0xf, 0),
            "ffff - f - 1 == ffef");
  assert.ok(1 === Alu.carry, "carry == 1");
...
QUnit.test("ALU BUS_PLUS_T_PLUS_1", function(assert) {
  ...
  // Carry should work.
  assert.ok(0xffff === Alu.execute(AluFunction.BUS_PLUS_T_PLUS_1, 0xfffd, 1, 0),
            "fffd + 1 + 1 == ffff");
  assert.ok(0 === Alu.carry, "carry == 0");
...
QUnit.test("ALU BUS_AND_NOT_T", function(assert) {
  assert.ok(4 === Alu.execute(AluFunction.BUS_AND_NOT_T, 5, 3, 0),
            "5 & ~3 == 4");
  assert.ok(0xd102 === Alu.execute(AluFunction.BUS_AND_NOT_T, 0xf13a, 0x2038),
           "f13a & ~2039 == d102")
